...
class MilRunner:
    ...
    def get_train_loader(self, data_list_key="training"):
        ...
        if self.mil_config["old_backend"]:
            backend = "tifffile"
        else:
            backend = "cucim"

        if self.mil_config["old_wsi"]:
            from monai.data.image_reader import WSIReader
        else:
            from monai.data.wsi_reader import WSIReader  # slower

        if self.mil_config["old_transform"]:

            train_transform = Compose(
                [
                    LoadImaged(keys=["image"], reader=WSIReader, backend=backend, dtype=None, level=1, image_only=True),
                    LabelEncodeIntegerGraded(num_classes=num_classes, keys=["label"]),
                    TileOnGridd(
                        keys=["image"],
            ...
            train_transform = Compose(
                [
                    ...
                    LoadImaged(
                        keys=["image"], reader=WSIReader, backend=backend, dtype=np.uint8, level=1, image_only=True
                    ),
    ...
    def get_val_loader(self, data_list_key="validation"):
        ...
        if self.mil_config["old_transform"]:
            valid_transform = Compose(
                [
                    LoadImaged(keys=["image"], reader=WSIReader, backend=backend, dtype=None, level=1, image_only=True),
                    LabelEncodeIntegerGraded(num_classes=num_classes, keys=["label"], allow_missing_keys=True),
                    TileOnGridd(
                        keys=["image"],
                        tile_count=None,
                        tile_size=tile_size,
